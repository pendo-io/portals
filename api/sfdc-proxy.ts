...
const LEAD_LIST_FIELDS = `Id, Name, FirstName, LastName, Company, Email, Website, Status, LeadSource, CreatedDate, Referral_Partner_Account__r.Name`;
...
const OPP_DETAIL_FIELDS = `Id, Name, Account.Name, Account.Website, Owner.Name, Owner.Email, Owner.Title, CreatedBy.Name,
                ...
                Initial_Contact__c, Initial_Contact__r.Name, Initial_Contact__r.Title, Initial_Contact__r.Email, Initial_Contact_Role__c`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const retryAfter = _rl(req, "sfdc-proxy", 300, 5 * 60 * 1000);
  if (retryAfter !== null) {
